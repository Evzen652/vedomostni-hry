...
21. 9. dokončeny Malajsie, Pákistán, Portugalsko, Saúdská Arábie, Dánsko, Indonésie, Norsko a Filipíny; bez ilustrace
zbývá **912** otázek. Klíč `GEMINI_API_KEY` (nový formát `AQ.…`) je v `.dev.vars`
worktree `pokracujeme-e79708` — v novém worktree nebo na jiném počítači chybí a hráč ho
musí vložit znovu. **Další země (Argentina, Belgie, Finsko…) nemají `irony_prompt`**,
...
Portugalsko, Saúdská Arábie, Dánsko, Indonésie, Norsko a Filipíny. Bez ilustrace je **912** otázek z 3 742.

**Další krok:** napsat `irony_prompt` pro Argentinu (pak další) v session, `npm run lint-irony`
(0 chyb), pak `node scripts/batch-irony-images.js submit --cc ar`. Na hromadný zápis zadání
